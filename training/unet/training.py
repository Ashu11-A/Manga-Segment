...
async def unetTraining(args: list[str], model: int | None):
	...
	def display(display_list, name: str = None):
		...
		for i in range(len(display_list)):
			...
			plt.axis("off")
		plt.savefig(
			f"model-test/{totalModel}/{name or datetime.now().strftime('%Y%m%d-%H%M%S')}.png"
		)
		plt.close()
	...
	def show_predictions(epoch: int, val_loss: int, val_accuracy: int):
		display(
			...
			name=f"{epoch}-[{val_loss}]-[{val_accuracy}]",
		)
	...
	model_history = model.fit(
		...
		callbacks=[
			ModelCheckpoint(
				f"models/my-model-{totalModel}/best_model",
	...
	keras.models.save_model(
		model, filepath=f"models/my-model-{totalModel}", overwrite=True
	)
