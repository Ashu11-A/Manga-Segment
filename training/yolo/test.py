...
def segment_images(model_num=None, image_size=1280):
		...
		for result in results:
			...
			for i, mask in enumerate(masks):
				...
				mask_file = os.path.join(
					output_dir,
					f"{os.path.splitext(image_name)[0]}_{class_name}_mask.png",
				)
			...
			output_file = os.path.join(
				output_dir, f"{os.path.splitext(image_name)[0]}_segmented.png"
			)
